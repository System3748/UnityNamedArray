...
public class NamedArrayDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        {
            if (namedInfo.fieldName != null)
            {
                label.text = $"{namedInfo.customName} {index} [{property.FindPropertyRelative(namedInfo.fieldName)?.stringValue}]";
            }
            else
            {
                if (namedInfo.isFormat)
                    label.text = string.Format(namedInfo.customName, index + namedInfo.enumOffest);
                else
                    label.text = $"{namedInfo.customName}-{index + namedInfo.enumOffest}";
            }
        }
        else if (namedInfo.fieldName != null)
        {
            label.text = $"Element {index} [{property.FindPropertyRelative(namedInfo.fieldName)?.stringValue}]";
        }
